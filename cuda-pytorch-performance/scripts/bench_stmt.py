#!/usr/bin/env python3
"""Benchmark a Python statement with optional CUDA synchronization."""
...
def summarize(samples_ms: list[float]) -> dict[str, Any]:
    ...
    if count > 1:
        summary["stdev_ms"] = statistics.stdev(sorted_samples)
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stmt",
        required=True,
        help="Python statement to benchmark. Example: 'model(x)' or 'loss.backward()'",
    )
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Emit only JSON instead of a human-readable summary.",
    )
